...
export class TaskMySuffixService {
  ...
  update(task: ITaskMySuffix): Observable<EntityResponseType> {
    return this.http.put<ITaskMySuffix>(`${this.resourceUrl}/${getTaskMySuffixIdentifier(task) as string}`, task, { observe: 'response' });
  }

  partialUpdate(task: ITaskMySuffix): Observable<EntityResponseType> {
    return this.http.patch<ITaskMySuffix>(`${this.resourceUrl}/${getTaskMySuffixIdentifier(task) as string}`, task, {
      observe: 'response',
    });
  }
